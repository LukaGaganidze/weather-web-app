...
export class WeatherFiveDayServiceInputService {
  constructor(private http: HttpClient) {}

  // RECIEVES COORDS AND RETURNS DATA FOR 5 DAYS FOR THE LOCATION
  getWeatherDataForFivedaysInput(input: string) {
    return this.http
      .get<WeatherDataForFiveDaysFromServer>(
        `https://api.openweathermap.org/data/2.5/forecast?q=${input}&appid=${environment.apiKey}&units=metric`
      )
      .pipe(
